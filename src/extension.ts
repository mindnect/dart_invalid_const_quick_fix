...
class InvalidConstantFixProvider implements vscode.CodeActionProvider {
  ...
  provideCodeActions(
    ...
    const quickFixActions = context.diagnostics
      .filter((diagnostic) => {
        if (typeof diagnostic.code === "object" && "value" in diagnostic.code) {
          return (
            diagnostic.code.value === "const_with_non_constant_argument" ||
            diagnostic.code.value === "const_constructor_with_field_initialized_by_non_const"
          );
        } else {
          return diagnostic.code === "invalid_constant";
        }
      })
      .map((diagnostic) => this.createFix(document, diagnostic, vscode.CodeActionKind.QuickFix));

    const sourceFixAllActions = context.diagnostics
      .filter((diagnostic) => {
        this.fixedIndex = [];
        if (typeof diagnostic.code === "object" && "value" in diagnostic.code) {
          return (
      ...
      .map((diagnostic) => this.createFix(document, diagnostic, vscode.CodeActionKind.SourceFixAll));
  ...
  private createFix(
    ...
    const constIndex = documentText.lastIndexOf(constKeyword, document.offsetAt(errorStartPosition));

    if (constIndex !== -1 && !this.fixedIndex.includes(constIndex)) {
      this.fixedIndex.push(constIndex);
      // Get the text from 'const' to the end of the error range
      const constRange = new vscode.Range(document.positionAt(constIndex), errorRange.end.translate(0, 1));
